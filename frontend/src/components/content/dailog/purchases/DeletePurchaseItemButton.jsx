...
import { useDeletePurchaseItem } from "@/hook/purchaseHook"
import { InlineIcon } from "@iconify/react"
import { useEffect } from "react"
import { useNavigate } from "react-router-dom"
import { toast } from "sonner"

const DeletePurchaseItemButton = ({name,uid, p_uid}) => {
  const nav = useNavigate()
  const deleteMutate = useDeletePurchaseItem(uid)
  ...
  useEffect(()=>{
    if(deleteMutate.isSuccess){
      nav(`/purchase/${p_uid}`)
      toast.success(`Purchase detail ${name} has ben delete succsessfully`)
    }
    ...
    <AlertDialog>
      ...
      <Button className="hover:bg-white cursor-pointer  py-5 border-1 border-gray-300 foucus:ring-1  ring-offset-2 hover:text-rose-700 hover:border-rose-700"  variant="outline"> <span> <InlineIcon icon={"fluent:delete-16-filled"}/></span>Delete</Button>
        ...
        <ConfirmPopupContainer 
          ...
          description="This action cannot be undone. This will permanently delete the detail and remove the data on the server."
...
export default DeletePurchaseItemButton
